...
.then(parcelsData => {
    ...
    parcelsFeatures.features.forEach(feature => {
        let info = parcelInfo.find(pinfo => pinfo.fields.parcel == feature.properties.pk);
        if(info) {
...
function listEventListener(e) {
    ...
    console.log(parcelNo, name);
    // update search input value
    searchInput.value = name;
    searchResult.innerHTML = "";

    // zoom to road layer
    data.features = data.features.filter(feature => feature.properties.pk == parseInt(parcelNo));
    console.log(data);
